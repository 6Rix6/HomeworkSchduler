    ...
    <View className="p-5 pt-2 w-screen">
      ...
      >
        <View className={`h-10 border-b  ${colorScheme == "dark"?`border-gray-300`:`border-gray-900`} flex flex-row items-center`}>{/** ヘッダー */}
            <Text className={`font-bold text-2xl mt-auto mb-auto ml-5 ${colorScheme == "dark"?`text-white`:`text-black`}`}>Tasks</Text>
            <View className='flex-1 w-full items-end'>
              <TouchableOpacity
                className='flex flex-row items-end'>
                  <Text className={`font-bold text-lg mt-auto mb-auto ${colorScheme == "dark"?`text-white`:`text-black`}`}>All Tasks</Text>
